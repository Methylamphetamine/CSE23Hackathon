...
def get_linear_ode(A, BT, BH, hH, DE, DH):
    ...
    def ode(t, x, u):
        ...
        cH = cooling per room
        '''
        # outside temp + solar radiation + humans per room + total heating per room + window ac per room
        eT = u[:1] # (outside temp)
        eH = u[1] # solar radiation
...
BT = opt.variable(n_rooms, 1) # 2: outside temperature + ground
...
DE = casadi.diag(opt.variable(n_rooms, 1)) # total heat
DH = casadi.diag(opt.variable(n_rooms, 1)) # window ac
...
u_dim = 1+1+n_rooms+n_rooms+n_rooms
dt = 10*60
...
#%%
np.save("linear_model_bd2_matrices_small", matrix_dict, allow_pickle=True)
